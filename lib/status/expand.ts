...
import { eachBusinessDay } from "@/lib/leave/calc";
 ...
 * - 반차: 해당일이 그 달 안이면 1행(0.5). 주말이어도 등록됐다면 표기.
 * - 그 외(연차·병가): 월 경계로 자른 뒤 영업일마다 1행(1).
...
export function expandLeaveUsage(
  ...
  for (const r of reqs) {
    ...
    for (const day of eachBusinessDay(from, to)) {
